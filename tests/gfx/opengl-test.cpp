...
int main() {
    ...
    auto gbuffer_shader =
        gfx::shader_program(gbuffer_vertex_shader_src, gbuffer_fragment_shader_src);
    ...
    std::string quad_fragment_shader_src = R"(
        ...
        uniform int displayMode; // 0 = position, 1 = normal, 2 = albedo, 3 = depth
    ...
    auto quad_shader = gfx::shader_program(quad_vertex_shader_src, quad_fragment_shader_src);
    ...
    // Normal color buffer
    auto gNormal_opt = gfx::texture2d::create_empty(
        window_width, window_height, GL_RGB16F, GL_RGB, GL_FLOAT);
    if (!gNormal_opt) {
    ...
    auto depth_texture_opt = gfx::texture2d::create_empty(
        window_width, window_height, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_FLOAT);
    ...
    if (!gbuffer.attach_texture(GL_COLOR_ATTACHMENT0, gPosition)) {
        std::cerr << "Failed to attach gPosition texture to framebuffer." << std::endl;
        return -1;
    }

    if (!gbuffer.attach_texture(GL_COLOR_ATTACHMENT1, gNormal)) {
        std::cerr << "Failed to attach gNormal texture to framebuffer." << std::endl;
        return -1;
    }

    if (!gbuffer.attach_texture(GL_COLOR_ATTACHMENT2, gAlbedoSpec)) {
        std::cerr << "Failed to attach gAlbedoSpec texture to framebuffer." << std::endl;
        return -1;
    }
    ...
    gfx::vertex_layout quadLayout = gfx::vertex_layout_builder()
        .with_attribute(gfx::vertex_attribute_semantic::position, gfx::vertex_attribute_type::float32, 2)
        .with_attribute(gfx::vertex_attribute_semantic::tex_coord0, gfx::vertex_attribute_type::float32, 2)
        .build();
    ...
    while (running) {
        ...
        pipeline.submit(gfx::set_uniform_cmd<float>("near", near_plane, quad_shader));
        pipeline.submit(gfx::set_uniform_cmd<float>("far", far_plane, quad_shader));

        // Set the display mode
        // 0 = Position, 1 = Normal, 2 = Albedo, 3 = Depth
        int displayMode = 3; // Change this value to switch between different textures
        pipeline.submit(gfx::set_uniform_cmd<int>("displayMode", displayMode, quad_shader));
